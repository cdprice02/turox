...
use turox_chess::{File, Move, MoveFlags, Piece, Rank, Square};
...
fn parse_san(token: &str) -> Option<SanMove> {
    ...
    let (token, promotion) = if bytes.len() >= 2 && bytes[bytes.len() - 2] == b'=' {
        // SAN's own rule: a promotion target is written uppercase, and can
        // only be one of the four pieces a pawn may become. The letter itself
        // comes from `turox-chess`; the two restrictions are notation's.
        let piece = match Piece::from_letter(char::from(bytes[bytes.len() - 1])) {
            Some(p @ (Piece::Knight | Piece::Bishop | Piece::Rook | Piece::Queen))
                if bytes[bytes.len() - 1].is_ascii_uppercase() =>
            {
                p
            }
            _ => return None,
    ...
    // SAN's own rule: a leading piece letter is uppercase, and a pawn has no
    // letter at all, so anything else means the token starts with a square.
    let (piece, front) = match front.split_at_checked(1) {
        Some((head, rest)) => match head.chars().next().and_then(|c| {
            c.is_ascii_uppercase()
                .then(|| Piece::from_letter(c))
                .flatten()
        }) {
            Some(Piece::Pawn) | None => (Piece::Pawn, front),
            Some(p) => (p, rest),
        },
        None => (Piece::Pawn, front),
    };
