...
/// plies of legal play. `depth == 0` stays its own branch rather than falling
/// out of the `depth == 1` bulk-counting shortcut below it: `perft` is called
/// directly with `depth == 0` (`perft_zero_is_one_leaf`, and indirectly any
/// depth-1 call's own base case), and without that branch `depth - 1`
/// underflows `u32` before reaching the `depth == 1` check.
