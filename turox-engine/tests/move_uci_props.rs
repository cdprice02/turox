//! Property and concrete tests for `Move::to_uci`/`Move::from_uci`.
//!
//! `any_board()` never generates an en passant state (see its own doc), so
//! the property test below never exercises that flag; a concrete FEN-based
//! test covers it directly instead, the same split `pseudo_legal_props.rs`
//! uses for en passant generation itself.
